...
- **UR-RL-007:** The system MUST offer a fast-evolve play mode (feature 021, `--mode fastevolve`; ADR-020) that adapts daily on failure: on a day-failure or near-failure trigger it pauses the game, runs a bounded reflection pass, promotes a gated candidate mid-run, reloads the day's anchor autosave, and reinitializes run state — at most the pack-configured reload budget per day (default 2), after which the day proceeds and the last evolved pack carries forward. The mode is unscored-only (scored episodes refuse it at launch), keeps fair protections on (debug-class methods stay refused; save/load is a scoped grant through the single-writer dispatch path), is never the default, and holds every retry-policy knob in pack data (`fastevolve:` section).
