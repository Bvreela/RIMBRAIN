...
- [x] T090d vitals hardening (research-agent findings): PawnBrief flag/roster-delta death detection (colonist_death can fire), needs-deprivation + bleeding/tending/mental-state counts, summary passthroughs (food_days/medicine/power/fires/threat_points/alerts), `colony.letter` letter-diff events; `drop_rule` reaches emergency+combat rules; ops-mutation candidates promote on metrics ties (they can't move dispatch metrics); 23 failure classes in improve-v0
