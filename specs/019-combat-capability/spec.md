...
- **FR-1905**: The combat state machine MUST be pack-data modes — watch, engage, hold, overrun, retreat, release — with hysteresis (release only after a hostile-free window; relief only below need-floor AND clear of nearby hostiles AND not overrun; re-draft only above recover threshold).
...
- **SC-1903**: With the selector endpoint offline, compiled combat assignments match the shadow-mode pick ≥80% of polls (deterministic fallback quality).
