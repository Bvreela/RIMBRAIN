...
- **FR-1107**: Brain reset — a bounded control channel `state/brain_reset.request` honored only when the runtime is launched with `--live-brain`. On request the loop re-validates and reloads the active pack file, clears mode/planning state (`startmode.json`, rule state), re-observes the colony, and re-derives goals from the fresh pack. The reset emits a `brain.reset` event and a decision-matrix row (`source: ui:brain-reset`), and writes `state/brain_status.json` (`ok`, `pack_revision`, `error`). Without `--live-brain` the request is refused in `brain_status.json`; scored/immutable runs never set the flag, preserving UR-BRN pack immutability. A failed reload keeps the run alive but fail-closed: the stale loaded hash vs the edited file trips `dispatch.pack_drift` until a valid pack is restored.
- **FR-1108**: The overlay gains a **Brain Reset** button (writes the request file) and a **brain pack editor** — a Tk text editor over the active pack YAML (`--pack-file`). "Save & Reset" writes the file then posts a reset request, so new parameters and priorities take effect live without restarting the process. The editor shows `brain_status.json` results; an invalid pack reports the validation error and the run stays fail-closed.
...
- **SC-1107**: Live run with `--live-brain`: editing a pack parameter via the editor + Brain Reset takes effect without process restart — the reloaded hash appears in `planning.json`/`brain_status.json`, goals re-derive, and a `ui:brain-reset` row appears in the matrix.
- **SC-1108**: An invalid pack edit is refused — `brain_status.json` reports the error, no game writes occur from the invalid policy, and the run stays fail-closed until a valid pack is restored.
