...
017 unified phase engine (ADR-019, FR-1401..1430): one runtime loop — observe → vitals → ledger reconcile → reflexes → rules → **plan stage** (`runtime/planstage.py`, `decide.plan`, 150 s cadence + phase-boundary/event triggers, gated `goal_order`/`activate`/`deactivate`/`promote`, one plan in force with staleness) → **select stage** (`runtime/select.py`, `decide.select`, ≤20-item candidate lists across colony+pawn scope, `priority_head` fallback, `caller=None` never probes an endpoint) → **phase execution** (`runtime/phase.py`, prescriptive init → phase goals → standing goals; plan deactivation filter + promoted options materialized in `RunState.plan`, never the pack file) → views → **reflect stage** (`runtime/evolve.py` — one digest→propose→gate→candidate→boundary-promotion pipeline over whitelisted surfaces `phases`/`action_list`/`decide`/`reflexes`/`rules`/`options`/`senses`/`metrics`; v0 op paths rewrite through the T006 migration map). `RunState` consolidates vars/phase/plan/rule/improve/vitals state. Packs: `schema_version: 1` native; v0 packs auto-migrate at load (`templates.migrate_v0` + `@cfg:` aliases). CLI: `--mode run|cycle|improve` × `--game sim|live` × `--stage plan|reflect|combat`; deprecated aliases (`sim`/`live`/`start`/`combat`) warn; cycle/combat require `--dev`; `--live-mutate` only on `run --game live`. Sim contract: `SimGame` + fixed clock + zero network → byte-identical event streams. Deleted: `universal.py`, `combatmode.py`, `mutate.py` (→ `evolve.py`); `startmode.py` is a compat shim; `improve.py` is evidence-only (`diagnose`/`score`/`predict_metrics`).
